  ...
  server.registerTool(
    ...
    async ({ query, limit }) => {
      try {
        ...
        if (results.length === 0) return text(`No CourtListener opinions match "${query}".`)
  ...
  server.registerTool(
    ...
    async ({ query, limit }) => {
      try {
        ...
        if (results.length === 0) return text(`No CourtListener dockets match "${query}".`)
...
function errorMessage(e: unknown): string {
  if (e instanceof CourtListenerError) return `Error: ${e.message}`
