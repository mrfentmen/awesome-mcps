...
export function createServer(): McpServer {
  ...
  server.registerTool(
    ...
    {
      title: "Search",
      description: "Search Zenodo records.",
  ...
  server.registerTool(
    "record",
    {
      ...
      inputSchema: z.object( { id: z.number().describe("Record ID.") }),
      annotations: READ_ONLY,
    },
    async (args) => {
    try { return text(await record(args)) } catch (e) { return textError(error(e)) }
  }
  )
  return server
}
