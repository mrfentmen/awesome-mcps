...
export async function searchListings(query: string, limit?: string): Promise<string> {
  ...
  return pretty({ brand: "Bottega Veneta", query, count: out.length, listings: out.slice(0, n) })
}
...
export async function getListing(handle: string, source?: string): Promise<string> {
  ...
  for (const s of sources) {
    try {
      return pretty({ brand: "Bottega Veneta", source: s.name, product: await getJson(s.product(handle)) })
...
export async function priceOverview(limit?: string): Promise<string> {
  ...
  return pretty({
    brand: "Bottega Veneta", currency: "USD", sample: prices.length,
