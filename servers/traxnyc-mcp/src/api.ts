export class TraxnycError extends Error {
  constructor(message: string) {
    super(message)
    this.name = "TraxnycError"
  }
}
...
const CATALOG = "https://traxnyc.com/products.json"
const SUGGEST = "https://traxnyc.com/search/suggest.json"
const PRODUCT_BASE = "https://traxnyc.com/products"
...
async function getJson(url: string): Promise<any> {
  ...
  try {
    ...
    throw new TraxnycError(`Network error: ${(e as Error).message}`)
  }
  if (!res.ok) throw new TraxnycError(`HTTP ${res.status} for ${new URL(url).hostname}`)
  return res.json()
}
...
export async function searchProducts(query: string, limit?: string, maxPrice?: string, productType?: string): Promise<string> {
  ...
  return pretty({ store: "TraxNYC", query, count: out.length, listings: out })
}
...
export async function listNewest(limit?: string, page?: string, productType?: string): Promise<string> {
  ...
  return pretty({ store: "TraxNYC", page: pg, count: out.length, listings: out.slice(0, n) })
}
...
export async function getProduct(handle: string): Promise<string> {
  ...
  return pretty({
    store: "TraxNYC", ...sl, price_usd: dollar, variants, images,
...
export async function findDeals(limit?: string, sample?: string): Promise<string> {
  ...
  if (!all.length) throw new TraxnycError("No available priced products found.")
  ...
  return pretty({
    store: "TraxNYC", sample: all.length, median_usd: median, count: Math.min(deals.length, n),
    deals: deals.slice(0, n),
    note: "Below-median asks, not appraisals. Not affiliated with TraxNYC.",
  })
}
...
export async function listCategories(): Promise<string> {
  ...
  return pretty({ store: "TraxNYC", sample: Object.values(counts).reduce((a, b) => a + b, 0), categories: counts })
}
