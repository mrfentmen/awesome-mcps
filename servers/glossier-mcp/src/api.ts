export class GlossierError extends Error {
  constructor(message: string) {
    super(message)
    this.name = "GlossierError"
  }
}
...
const CATALOG = "https://www.glossier.com/products.json"
const SUGGEST = "https://www.glossier.com/search/suggest.json"
const PRODUCT_BASE = "https://www.glossier.com/products"
...
async function getJson(url: string): Promise<any> {
  ...
  try {
    ...
    throw new GlossierError(`Network error: ${(e as Error).message}`)
  }
  if (!res.ok) throw new GlossierError(`HTTP ${res.status} for ${new URL(url).hostname}`)
  return res.json()
}
...
export async function searchProducts(query: string, limit?: string, maxPrice?: string, productType?: string): Promise<string> {
  ...
  return pretty({ store: "Glossier", query, count: out.length, listings: out })
}
...
export async function listNewest(limit?: string, page?: string, productType?: string): Promise<string> {
  ...
  return pretty({ store: "Glossier", page: pg, count: out.length, listings: out.slice(0, n) })
}
...
export async function getProduct(handle: string): Promise<string> {
  ...
  return pretty({
    store: "Glossier", ...sl, price_usd: dollar, variants, images,
...
export async function findDeals(limit?: string, sample?: string): Promise<string> {
  ...
  if (!all.length) throw new GlossierError("No available priced products found.")
  ...
  return pretty({
    store: "Glossier", sample: all.length, median_usd: median, count: Math.min(deals.length, n),
    deals: deals.slice(0, n),
    note: "Below-median asks, not appraisals. Not affiliated with Glossier.",
  })
}
...
export async function listCategories(): Promise<string> {
  ...
  return pretty({ store: "Glossier", sample: Object.values(counts).reduce((a, b) => a + b, 0), categories: counts })
}
