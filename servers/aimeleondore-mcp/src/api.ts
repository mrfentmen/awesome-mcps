export class AimeLeonDoreError extends Error {
  constructor(message: string) {
    super(message)
    this.name = "AimeLeonDoreError"
  }
}
...
const CATALOG = "https://aimeleondore.com/products.json"
const SUGGEST = "https://aimeleondore.com/search/suggest.json"
const PRODUCT_BASE = "https://aimeleondore.com/products"
...
async function getJson(url: string): Promise<any> {
  ...
  try {
    ...
    throw new AimeLeonDoreError(`Network error: ${(e as Error).message}`)
  }
  if (!res.ok) throw new AimeLeonDoreError(`HTTP ${res.status} for ${new URL(url).hostname}`)
  return res.json()
}

function slim(p: any, cents = false): Slim {
...
export async function searchProducts(query: string, limit?: string, maxPrice?: string, productType?: string): Promise<string> {
  ...
  return pretty({ store: "Aimé Leon Dore", query, count: out.length, listings: out })
}
...
export async function listNewest(limit?: string, page?: string, productType?: string): Promise<string> {
  ...
  return pretty({ store: "Aimé Leon Dore", page: pg, count: out.length, listings: out.slice(0, n) })
}
...
export async function getProduct(handle: string): Promise<string> {
  ...
  return pretty({
    store: "Aimé Leon Dore", ...sl, price_usd: dollar, variants, images,
...
export async function findDeals(limit?: string, sample?: string): Promise<string> {
  ...
  if (!all.length) throw new AimeLeonDoreError("No available priced products found.")
  ...
  return pretty({
    store: "Aimé Leon Dore", sample: all.length, median_usd: median, count: Math.min(deals.length, n),
    deals: deals.slice(0, n),
    note: "Below-median asks, not appraisals. Not affiliated with Aimé Leon Dore.",
  })
}
...
export async function listCategories(): Promise<string> {
  ...
  return pretty({ store: "Aimé Leon Dore", sample: Object.values(counts).reduce((a, b) => a + b, 0), categories: counts })
}
