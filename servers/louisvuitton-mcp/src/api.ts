export class LouisVuittonError extends Error {
  constructor(message: string) {
    super(message)
    this.name = "LouisVuittonError"
  }
}
...
const VENDORS: string[] = ["louis vuitton"]
...
async function getJson(url: string): Promise<any> {
  ...
  try {
    ...
    throw new LouisVuittonError(`Network error: ${(e as Error).message}`)
  }
  if (!res.ok) throw new LouisVuittonError(`HTTP ${res.status} for ${new URL(url).hostname}`)
  return res.json()
}
...
export async function searchListings(query: string, limit?: string, maxPrice?: string, condition?: string): Promise<string> {
  ...
  if (condition !== undefined && cond === null) throw new LouisVuittonError(`Unknown condition '${condition}'. Use: ${CONDITIONS.join(", ")}.`)
  const out: Slim[] = []
  for (const s of SOURCES) {
    try {
  ...
  return pretty({ brand: "Louis Vuitton", query, count: out.length, condition_filter: cond ?? null, listings: out.slice(0, n) })
}
...
export async function listNewest(limit?: string, page?: string, productType?: string, maxPrice?: string, condition?: string): Promise<string> {
  ...
  return pretty({ brand: "Louis Vuitton", page: pg, count: out.length, listings: out.slice(0, n) })
}
...
export async function getListing(handle: string, source?: string): Promise<string> {
  ...
  if (!sources.length) throw new LouisVuittonError(`Unknown source '${source}'. Use fashionphile or rebag.`)
  for (const s of sources) {
    try {
      ...
      return pretty({
        brand: "Louis Vuitton", ...sl, price_usd: dollar, variants, images,
        description: html.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().slice(0, 1500) || null,
        tags: p.tags,
      })
    } catch { /* try next source */ }
  }
  throw new LouisVuittonError(`Listing '${handle}' not found on ${sources.map((s) => s.name).join(", ")}.`)
}
...
export async function priceOverview(limit?: string): Promise<string> {
  ...
  if (!prices.length) throw new LouisVuittonError("No priced listings found.")
  ...
  return pretty({
    brand: "Louis Vuitton", currency: "USD", overall: stats(prices), by_condition: condStats,
    note: "Asking prices on resale sources, not appraisals. Not affiliated with Louis Vuitton.",
  })
}
...
export async function comparePrices(query: string, limit?: string): Promise<string> {
  ...
  return pretty({ brand: "Louis Vuitton", query, groups: rows.length, rows: rows.slice(0, 15) })
}
...
export async function findDeals(limit?: string, sample?: string): Promise<string> {
  ...
  if (!all.length) throw new LouisVuittonError("No available priced listings found.")
  ...
  return pretty({
    brand: "Louis Vuitton", sample: all.length, brand_median_usd: median, count: Math.min(deals.length, n),
    deals: deals.slice(0, n),
    note: "Below-median asks, not appraisals or authenticity guarantees. Not affiliated with Louis Vuitton.",
  })
}
...
export async function brandOverview(): Promise<string> {
  ...
  return pretty({
    brand: "Louis Vuitton", catalog_sample: total, by_source: bySource,
    by_product_type: byType, by_condition: byCondition,
    note: "Sample of newest ~500 listings per source. Not affiliated with Louis Vuitton.",
  })
}
