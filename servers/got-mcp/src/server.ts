import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js"
import { z } from "zod"
import { book } from "./api.js"
import { character } from "./api.js"
import { houses } from "./api.js"

const text = (value: string) => ({ content: [{ type: "text" as const, text: value }] })
const error = (e: unknown) => `Error: ${e instanceof Error ? e.message : String(e)}`

export function createServer(): McpServer {
  const server = new McpServer({ name: "got-mcp", version: "1.0.0" })
  server.tool("book", "Get a book by id.", { id: z.number().describe("Book id.") }, async (args) => {
    try { return text(await book(args)) } catch (e) { return text(error(e)) }
  })
  server.tool("character", "Get a character by id.", { id: z.number().describe("Character id.") }, async (args) => {
    try { return text(await character(args)) } catch (e) { return text(error(e)) }
  })
  server.tool("houses", "List houses.", { limit: z.number().describe("Max results.").optional() }, async (args) => {
    try { return text(await houses(args)) } catch (e) { return text(error(e)) }
  })
  return server
}
