...
export async function getCategories() {
    ...
    if (!res.ok)
        throw new TriviaError(`Trivia API error ${res.status}`);
    const d = await res.json();
    return d?.trivia_categories ?? [];
}
export async function getQuestions(amount = 10, category, difficulty, type) {
    ...
    if (d.response_code === 1)
        throw new TriviaError("No questions match those filters. Try another category or difficulty.");
    if (d.response_code === 2)
        throw new TriviaError("Invalid parameter.");
    if (d.response_code !== 0)
        throw new TriviaError(`Trivia API response code ${d.response_code}.`);
...
export function formatQuestion(q, index) {
    const lines = [
