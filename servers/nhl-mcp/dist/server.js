...
export function createServer() {
    ...
    server.tool("get_team_stats", "Season team stats: wins, losses, points, goals, special teams.", { season: z.string().default("20252026").describe("Season id like '20252026'"), limit: z.number().int().min(1).max(32).default(10) }, async ({ season, limit }) => {
        try {
    ...
    server.tool("get_standings", "Current NHL standings for every team.", {}, async () => {
        try {
    ...
    server.tool("get_schedule", "Today's NHL games with scores where played.", {}, async () => {
        try {
