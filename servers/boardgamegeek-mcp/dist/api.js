...
async function fetchXml(path, retries = 3) {
    let lastStatus = 0;
    for (let attempt = 0; attempt < retries; attempt++) {
        ...
        if (res.status === 401 || res.status === 403) {
            throw new BggError("BoardGameGeek refused the request (HTTP 401/403). BGG throttles datacenter IPs; try again later or from a residential IP.");
        }
        if (!res.ok)
            throw new BggError(`BoardGameGeek error ${res.status}`);
...
export function parseThing(xml) {
...
export async function searchGames(query, exact = false, limit = 5) {
    const xml = await fetchXml(`/search?query=${encodeURIComponent(query)}&type=boardgame,boardgameexpansion${exact ? "&exact=1" : ""}`);
    return parseSearch(xml).slice(0, limit);
}
export async function getGame(id) {
    const clean = id.trim();
    if (!/^\d+$/.test(clean))
        throw new BggError(`Game id must be numeric, got "${id}". Search first to find the id.`);
    const xml = await fetchXml(`/thing?id=${clean}&stats=1`);
    return parseThing(xml);
}
export async function hotGames(limit = 10) {
