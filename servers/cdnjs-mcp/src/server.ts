...
export function createServer(): McpServer {
  ...
  server.registerTool(
    ...
    {
      title: "Search",
      description: "Search cdnjs libraries.",
  ...
  server.registerTool(
    ...
    {
      title: "Library",
      description: "Details for one library.",
      inputSchema: z.object( { name: z.string().describe("Library name like react.") }),
      annotations: READ_ONLY,
    },
    async (args) => {
    try { return text(await library(args)) } catch (e) { return textError(error(e)) }
  }
  )
  return server
}
