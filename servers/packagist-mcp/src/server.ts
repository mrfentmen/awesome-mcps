import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js"
import { z } from "zod"
import { packageInfo } from "./api.js"
import { searchPackages } from "./api.js"

const text = (value: string) => ({ content: [{ type: "text" as const, text: value }] })
const textError = (t: string) => ({ content: [{ type: "text" as const, text: t }], isError: true as const })
const READ_ONLY = { readOnlyHint: true, openWorldHint: true } as const
const error = (e: unknown) => `Error: ${e instanceof Error ? e.message : String(e)}`

export function createServer(): McpServer {
  const server = new McpServer({ name: "packagist-mcp", version: "1.0.0" })
  server.registerTool(
    "package_info",
    {
      title: "Package info",
      description: "Get details for a PHP package.",
      inputSchema: z.object( { name: z.string().describe("Package name like laravel/framework.") }),
      annotations: READ_ONLY,
    },
    async (args) => {
    try { return text(await packageInfo(args)) } catch (e) { return textError(error(e)) }
  }
  )
  server.registerTool(
    "search_packages",
    {
      title: "Search packages",
      description: "Search packages.",
      inputSchema: z.object( { query: z.string().describe("Search terms."), limit: z.number().describe("Max results.").optional() }),
      annotations: READ_ONLY,
    },
    async (args) => {
    try { return text(await searchPackages(args)) } catch (e) { return textError(error(e)) }
  }
  )
  return server
}
